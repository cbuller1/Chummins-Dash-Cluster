...
static void _write_u32(uint16_t addr, uint32_t v) {
    uint8_t* p = (uint8_t*)&v;
    for (int i = 0; i < 4; i++) _fram.write(addr + i, p[i]);
}

static uint32_t _read_u32(uint16_t addr) {
    uint32_t v;
    uint8_t* p = (uint8_t*)&v;
    for (int i = 0; i < 4; i++) p[i] = _fram.read(addr + i);
    return v;
}

static void _write_double(uint16_t addr, double v) {
    uint8_t* p = (uint8_t*)&v;
    for (int i = 0; i < 8; i++) _fram.write(addr + i, p[i]);
}

static double _read_double(uint16_t addr) {
    double v;
    uint8_t* p = (uint8_t*)&v;
    for (int i = 0; i < 8; i++) p[i] = _fram.read(addr + i);
    return v;
}
